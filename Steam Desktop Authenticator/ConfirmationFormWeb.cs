...
namespace Steam_Desktop_Authenticator
{
    public partial class ConfirmationFormWeb : Form
    {
        ...
        public ConfirmationFormWeb(SteamGuardAccount steamAccount)
        {
            ...
            this.Text = String.Format("Подтверждения обмена - {0}", steamAccount.AccountName);
        }
        private async Task LoadData()
        {
            ...
            {
                MessageBox.Show("Ваша сессия истекла. Используйте кнопку \"Войти снова\" в меню выбранного аккаунта.", "Подтверждения обмена", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Close();
                return;
            }
            ...
            try
            {
                ...
                {
                    Label errorLabel = new Label() { Text = "Нет подтверждений для обработки", AutoSize = true, ForeColor = Color.Black, Location = new Point(150, 20) };
